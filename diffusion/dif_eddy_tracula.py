...
#tracula_dir = '/mindhive/xnat/data/autoxy/'
tracula_dir = '/mindhive/xnat/data/autoxy/tracula'

#subject_list = sorted([os.path.basename(x) for x in glob('/om/project/voice/processedData/openfmri/voice*')])
subject_list = sorted([os.path.basename(x) for x in glob('/mindhive/xnat/data/autoxy/ASD_0*')])
...
templates = {'dwi': '/mindhive/xnat/data/autoxy/{subject_id}/dmri/dwi_{PE}_001.nii.gz', 
              'bvals': '/mindhive/xnat/data/autoxy/{subject_id}/dmri/dwi_{PE}_001.bvals',
              'bvecs': '/mindhive/xnat/data/autoxy/{subject_id}/dmri/dwi_{PE}_001.bvecs'}
...
wf.base_dir = os.path.abspath('/om/scratch/Thu/dorit/')
#wf.run('SLURM', plugin_args={'sbatch_args': '--qos=saxelab -N1 -c2 --mem=10G'})
wf.config['execution']['poll_sleep_duration'] = 2

wf.run('SLURM',plugin_args={'sbatch_args': '-N1 -c2 --mem=10G'})
